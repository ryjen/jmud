...
public abstract class TelnetProtocolHandler {
	...
	private final static Logger log = LoggerFactory
			.getLogger(TelnetProtocolHandler.class);
	...
	private final static byte TELOPT_COMPRESS2 = (byte) 86;
	...
	public int negotiate(byte nbuf[]) throws IOException {
		...
		while (!dobreak && (boffset < count) && (noffset < nbuf.length)) {
			...
			case STATE_IACSBDATAIAC:
				...
				break;
			default:
				log.debug("This should not happen: " + neg_state + " ");
				neg_state = STATE_DATA;
				break;
			}
		}
	...
	public void setWindowSize(int columns, int rows) throws IOException {
		log.debug("sending NAWS");

		if (receivedDX[TELOPT_NAWS] != DO) {
			log.error("not allowed to send NAWS? (DONT NAWS)");
			return;
		}
		write(IAC);
		write(SB);
	...
	protected abstract void setMCCPRequest(boolean mccp);
