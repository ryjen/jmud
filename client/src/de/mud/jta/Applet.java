...
public class Applet extends JApplet {
		...
		if (pluginLoader == null) {
			try {
				...
				try {
					...
					log.error("try extracting it from the jar file");
					log.error("expected file here: " + getCodeBase()
							+ "default.conf");
				}
			}
			...
			if ((value = getParameter("config")) != null) {
				...
				try {
					...
					try {
						...
						log.error("could not find config file: " + ce);
					}
				}

				if (url != null) {
					try {
						appletParams.load(Applet.class
								.getResourceAsStream("/de/mud/jta/" + value));
						...
						try {
							...
							log.error("could not load config file: " + e2);
						}
					}
				}
			}
			...
			if (!(new Boolean(
					options.getProperty("Applet.disconnect.closeWindow"))
					.booleanValue()))
			...
			if ((new Boolean(options.getProperty("Applet.detach")))
					.booleanValue()) {
				if (frameTitle == null) {
					appletFrame = new JFrame("jta: " + host
							+ (port.equals("23") ? "" : " " + port));
				} else {
			...
			Map<String, JComponent> componentList = pluginLoader
					.getComponents();
			Iterator<String> names = componentList.keySet().iterator();
			while (names.hasNext()) {
				String name = names.next();
				Component c = componentList.get(name);
			...
			if (appletFrame != this) {
				final String startText = options
						.getProperty("Applet.detach.startText");
				final String stopText = options
						.getProperty("Applet.detach.stopText");
				final Button close = new Button();

				// this works for Netscape only!
				Vector<String> privileges = Common.split(
				...
				try {
					...
					enable = privilegeManager.getMethod("enablePrivilege",
							new Class[] { String.class });
				} catch (Exception e) {
					log.error("This is not Netscape ...");
				}
				...
				if (privilegeManager != null && enable != null
					...
					for (int i = 0; i < privileges.size(); i++)
						try {
							...
							log.info("access for '" + privileges.elementAt(i)
									+ "' allowed");

						} catch (Exception e) {
							log.error("access for '" + privileges.elementAt(i)
									+ "' denied");
						}

				// set up the clipboard
				try {
					clipboard = appletFrame.getContentPane().getToolkit()
							.getSystemClipboard();
					log.info("acquired system clipboard: " + clipboard);
				} catch (Exception e) {
				...
				if ((new Boolean(
						options.getProperty("Applet.detach.immediately"))
						.booleanValue())) {
					if ((new Boolean(
							options.getProperty("Applet.detach.fullscreen"))
							.booleanValue()))
						((JFrame) appletFrame).setSize(appletFrame
								.getContentPane().getToolkit().getScreenSize());
					else
						((JFrame) appletFrame).pack();

					((JFrame) appletFrame).setVisible(true);
					pluginLoader.broadcast(new SocketRequest(host, Integer
							.parseInt(port)));
				...
				close.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent evt) {
						if (((JFrame) appletFrame).isVisible()) {
							...
							close.setLabel(startText != null ? startText
									: "Connect");
						} else {
							if (frameTitle == null)
								((JFrame) appletFrame).setTitle("jta: " + host
										+ (port.equals("23") ? "" : " " + port));
							if ((new Boolean(options
									.getProperty("Applet.detach.fullscreen"))
									.booleanValue()))
								((JFrame) appletFrame).setSize(appletFrame
										.getContentPane().getToolkit()
										.getScreenSize());
							else
								((JFrame) appletFrame).pack();
							((JFrame) appletFrame).setVisible(true);
							...
							pluginLoader.broadcast(new ReturnFocusRequest());
							close.setLabel(stopText != null ? stopText
									: "Disconnect");
						}
					}
				});
				...
				tmp.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent evt) {
						if (pluginLoader.getComponents().get("Terminal") != null) {
							PrintJob printJob = appletFrame
									.getContentPane()
									.getToolkit()
									.getPrintJob((JFrame) appletFrame,
											"JTA Terminal", null);
							((Component) pluginLoader.getComponents().get(
									"Terminal")).print(printJob.getGraphics());
							printJob.end();
						}
					}
				});
				...
				tmp.addActionListener(new ActionListener() {
					@Override
					public void actionPerformed(ActionEvent evt) {
						log.debug("paste: " + focussedPlugin);
						if (focussedPlugin instanceof VisualTransferPlugin)
							((VisualTransferPlugin) focussedPlugin)
									.paste(clipboard);
					}
				...
				Map<String, JMenu> menuList = pluginLoader.getMenus();
				...
				tmp.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						Help.show(appletFrame.getContentPane(),
								options.getProperty("Help.url"));
					}
				});
				mb.setHelpMenu(help);
				...
				try {
					...
					log.error("could not set up Window event listener");
					log.error("you will not be able to close it");
				}
	...
	private void parameterOverride(Properties options) {
		...
		while (e.hasMoreElements()) {
			...
			if (value != null) {
				log.error("overriding value of " + key + " with " + value);
