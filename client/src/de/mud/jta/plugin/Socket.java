...
import de.mud.jta.event.MCCPListener;
...
public class Socket extends Plugin implements FilterPlugin, SocketListener {
	...
	public Socket(final PluginBus bus, final String id) {
		...
		bus.registerPluginListener(new MCCPListener() {
			@Override
			public void setMCCP(boolean on) {

				try {
					if (on) {
						ZInputStream zstr = new ZInputStream(socket
								.getInputStream());
						zstr.setFlushMode(JZlib.Z_SYNC_FLUSH);
						in = zstr;
						log.info("Starting MCCP...");
					} else {
						in = socket.getInputStream();
					}
				} catch (IOException e) {
					error = "Sorry, Could not set MCCP" + "\r\n" + "Reason: "
							+ e + "\r\n\r\n";
					error("can't set MCCP: " + e);
				}
			}
		});
	}
