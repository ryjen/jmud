...
public class URLFilter extends Plugin implements FilterPlugin, VisualPlugin,
	...
	protected Vector<String> protocols = new Vector<String>();
	...
	private Vector<String> urlCache = new Vector<String>();
	...
	public URLFilter(PluginBus bus, final String id) {
		...
		bus.registerPluginListener(new ConfigurationListener() {
			...
			public void setConfiguration(PluginConfig config) {
				...
				if ((s = config.getProperty("URLFilter", id, "protocols")) != null) {
					...
					while (idx >= 0) {
						log.info("adding protocol '"
								+ s.substring(old + 1, idx) + "'");
						protocols.addElement(s.substring(old + 1, idx));
						old = idx;
						idx = s.indexOf(',', old + 1);
					}
					log.info("adding protocol '" + s.substring(old + 1) + "'");
		...
		try {
			...
			log.error("error installing recognizer: " + e);
		}
	...
	public void run() {
		try {
			...
			log.error("recognition aborted: " + e);
		}
	}
	...
	protected void showURL(String url) {
		if (context == null) {
			log.error("no url-viewer available\n");
			return;
		}
		try {
