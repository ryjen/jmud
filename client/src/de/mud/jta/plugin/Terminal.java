...
public class Terminal extends Plugin implements FilterPlugin,
	...
	private final static Logger log = LoggerFactory.getLogger(Terminal.class);
	...
	public Terminal(final PluginBus bus, final String id) {
		...
		try {
			...
			log.info("Got the clipboard reference ok - copy & paste enabled");
		} catch (Exception ex) {
			log.error("Failed to get clipboard - copy and paste will not work");
			/* ex.printStackTrace(); */
		}
	...
	public void run() {
		...
		while (n >= 0)
			try {
				n = read(b);
				if (n > 0)
					log.debug("s\"" + (new String(b, 0, n, encoding)) + "\"");
				if (n > 0)
