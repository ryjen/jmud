...
public class ButtonBar extends Plugin implements FilterPlugin, VisualPlugin,
	...
	private static final Logger log = LoggerFactory.getLogger(ButtonBar.class);
					...
					while ((token = setup.nextToken()) != StreamTokenizer.TT_EOF) {
						...
						case StreamTokenizer.TT_WORD:
							...
							if (setup.sval.equals("button")) {
								...
								list.addListSelectionListener(ButtonBar.this);// Choices
								// use
								// ItemListener,
								// not
								// Action
								...
								panel.add(list);
								while ((token = setup.nextToken()) != StreamTokenizer.TT_EOF) {
									if (isKeyword(setup.sval)) {// Got command
										// ... Back off.
										setup.pushBack();
										break;
									}
									...
									if (token == StreamTokenizer.TT_EOF) {
										...
										if (isKeyword(value)) { // Missing
											// command -
											// complain but
											// continue
											log.error(descr
													+ ": missing choice command");
											setup.pushBack();
											break;
										}
										log.info("choice: name='" + descr
												+ "', value='" + value);
							...
							break;
						default:
							ButtonBar.this.error("syntax error at line "
									+ setup.lineno());
						}
					}
				} catch (IOException e) {
					ButtonBar.this
							.error("unexpected error while reading setup: " + e);
				}
				panel.validate();
			}
		});
	...
	public void actionPerformed(ActionEvent evt) {
		String tmp;
		if ((tmp = buttons.get(evt.getSource())) != null)
			processEvent(tmp);
	}
	...
	public void valueChanged(ListSelectionEvent evt) {
		String tmp, ident;
		if ((ident = choices.get(evt.getSource())) != null) {
			// It's a choice - get the text from the selected item
			JList list = (JList) evt.getSource();
			tmp = choices.get(ident + list.getSelectedValue());
			if (tmp != null)
				processEvent(tmp);
		}
	}
	...
	private void processEvent(String tmp) {
		...
		while ((idx = tmp.indexOf('\\', oldidx)) >= 0 && ++idx <= tmp.length()) {
			...
			case '@': {
				...
				idx = ni;
				JTextField t;
				if (fields == null || (t = fields.get(name)) == null) {
			...
			if (function.equals("connect")) {
				...
				try {
					if ((idx = cmd.indexOf(",")) >= 0) {
						try {
							port = Integer.parseInt(cmd.substring(idx + 1,
									cmd.length()));
						} catch (Exception e) {
							port = -1;
						}
