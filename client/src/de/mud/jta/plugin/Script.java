...
public class Script extends Plugin implements FilterPlugin {
	...
	private void setup(Vector<String[]> script) {
		...
		this.script = (Vector<String[]>) script.clone();
		log.debug("script contains " + script.size() + " elements");

		if (script.size() > 0) {
			// If the first element is empty, just send the value string.
			match = (this.script.firstElement())[0].getBytes();
			if (match.length == 0) {
				try {
					write(found());
				} catch (Exception e) {
