...
public class SshPacket2 extends SshPacket {
	...
	private final static Logger log = LoggerFactory.getLogger(SshPacket2.class);
	...
	public byte[] addPayload(byte buff[]) {
		...
		log.debug("addPayload2 " + buff.length);
		...
		while (boffset < buff.length) {
			...
			case PHASE_packet_length:
				...
				if (position == 5) {
					...
					position = 0;
					log.debug("SSH2: packet length " + packet_length);
					log.debug("SSH2: padlen " + padlen);
			...
			case PHASE_block:
				...
				if (position == block.length) { // the block is complete
					...
					for (int i = 0; i < decryptedBlock.length; i++)
						log.debug(" " + decryptedBlock[i]);

					setType(decryptedBlock[0]);
					log.debug("Packet type: " + getType());
					log.debug("Packet len: " + packet_length);
	...
	public byte[] getPayLoad(SshCrypto xcrypt, long seqnr) throws IOException {
		...
		log.debug("packet length is " + packet_length + ", padlen is " + padlen);
		if (xcrypt == null)
