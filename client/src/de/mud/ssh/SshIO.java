...
public abstract class SshIO {
	...
	static {
		try {
			...
			log.error("unable to load message digest algorithm: " + e);
			e.printStackTrace();
		}
	}
	...
	public byte[] handleSSH(byte buff[]) throws IOException {
		...
		log.debug("getPacket(" + buff + "," + buff.length + ")");
			...
			while (boffset < buff.length) {
				...
				if (b == '\n') {
					if (!idstr.substring(0, 4).equals("SSH-")) {
						...
						// idstr
						log.debug("Received data line: " + idstr);
						idstr = "";
						continue;
					}
					phase++;
					...
					log.debug("remotemajor " + remotemajor);
					log.debug("remoteminor " + remoteminor);
					...
					if (remotemajor == 2) {
						...
						/*
						 * if (false && (remoteminor == 99)) { mymajor = 2;
						 * myminor = 0; useprotocol = 2; } else
						 */{
							mymajor = 1;
							myminor = 5;
							useprotocol = 1;
	...
	public void setWindowSize(int columns, int rows) throws IOException {
		if (phase == PHASE_INIT) {
			log.error("setWindowSize(), sizing in init phase not supported.\n");
		}
		log.debug("setWindowSize(" + columns + "," + rows + ")");
	...
	private String handlePacket1(SshPacket1 p) throws IOException { // the
		...
		log.debug("1 packet to handle, type " + p.getType());
		...
		case SSH_SMSG_SUCCESS:
			log.debug("SSH_SMSG_SUCCESS (last packet was " + lastPacketSentType
					+ ")");
		...
		case SSH_SMSG_STDERR_DATA: // receive some error data from the server
			...
			log.debug("handlePacket : " + "STDERR_DATA " + str);
			return str;
		...
		case SSH_SMSG_EXITSTATUS: // sent by the server to indicate that
			...
			log.debug("Exit status " + value);
			disconnect();
			break;

		case SSH_MSG_DEBUG:
			str = p.getString();
			log.debug("handlePacket : " + " DEBUG " + str);
		...
		default:
			log.error("handlePacket1: Packet Type unknown: " + p.getType());
			break;
	...
	private String Send_SSH_CMSG_SESSION_KEY(byte[] anti_spoofing_cookie,
			...
			if ((supported_ciphers_mask[3] & (1 << SSH_CIPHER_IDEA)) != 0) {
				...
				if ((supported_ciphers_mask[3] & (1 << SSH_CIPHER_3DES)) != 0) {
					...
					if ((supported_ciphers_mask[3] & (1 << SSH_CIPHER_DES)) != 0) {
						...
						log.error("remote server does not supported IDEA, BlowFish or 3DES, support cypher mask is "
		...
		log.debug("Using " + cipher_type + " blockcipher.\n");
	...
	private String Send_SSH_CMSG_USER() throws IOException {
		log.debug("Send_SSH_CMSG_USER(" + login + ")");
	...
	/*
	 * private void write(byte b) throws IOException { one[0] = b; write(one); }
	 */
