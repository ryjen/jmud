...
public class SshPacket1 extends SshPacket {
	private final static Logger log = LoggerFactory.getLogger(SshPacket1.class);
		...
		while (boffset < buff.length) {
			...
			case PHASE_block:
				...
				if (position == block.length) { // the block is complete
					...
					if (!checkCrc())
						log.error("CRC wrong in received packet!");
	...
	private boolean checkCrc() {
		...
		log.debug(crc_arrayCheck[3] + " == " + crc_array[3]);
		log.debug(crc_arrayCheck[2] + " == " + crc_array[2]);
		log.debug(crc_arrayCheck[1] + " == " + crc_array[1]);
		log.debug(crc_arrayCheck[0] + " == " + crc_array[0]);
