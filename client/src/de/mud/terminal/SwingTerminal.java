...
public class SwingTerminal extends Component implements VDUDisplay,
		...
		if (ver >= 1.4) {
			...
			try {
				...
				SwingTerminal.class.getMethod("setFocusable", params).invoke(
						this, new Object[] { new Boolean(true) });
				SwingTerminal.class.getMethod("setFocusTraversalKeysEnabled",
						params).invoke(this,
						new Object[] { new Boolean(false) });
			} catch (Exception e) {
				log.error("vt320: unable to reset focus handling for java version "
						+ version);
				e.printStackTrace();
			}
		}
	...
	public void mouseDragged(MouseEvent evt) {
		if (buttonCheck(evt.getModifiers(), InputEvent.BUTTON1_MASK) ||
		// Windows NT/95 etc: returns 0, which is a bug
			...
			if (oldx != x || oldy != y) {
				...
				log.debug("select([" + selectBegin.x + "," + selectBegin.y
	...
	public void paint(Graphics g) {
		...
		log.debug("Clip region: " + g.getClipBounds());
	...
	public void print(Graphics g) {
		...
		if (!colorPrinting) {
			...
			colorSave = color;
			color = new Color[] { Color.black, Color.black, Color.black,
	...
	public void setBounds(int x, int y, int w, int h) {
		log.debug("VDU: setBounds(" + x + "," + y + "," + w + "," + h + ")");
		...
		log.debug("VDU: looking for better match for " + normalFont);
		...
		case RESIZE_FONT:
			...
			if (charHeight <= 1) {
				log.error("VDU: error during resize, resetting");
				normalFont = tmpFont;
				log.error("VDU: disabling font/screen resize");
		...
		log.debug("VDU: charWidth=" + charWidth + ", " + "charHeight="
				+ charHeight + ", " + "charDescent=" + charDescent);
	 ...
	 */
	/*
	 * private Color brighten(Color clr) { int r, g, b;
	 * 
	 * r = (int) min(clr.getRed() * 1.2, 255.0); g = (int) min(clr.getGreen() *
	 * 1.2, 255.0); b = (int) min(clr.getBlue() * 1.2, 255.0); return new
	 * Color(r, g, b); }
	 */
	...
	protected void redraw(Graphics g) {
		...
		for (int l = 0; l < buffer.height; l++) {
			...
			log.debug("redraw(): line " + l);
			for (int c = 0; c < buffer.width; c++) {
				int addr = 0;
				int currAttr = 0;

				try {
					currAttr = buffer.charAttributes[buffer.windowBase + l][c];
				} catch (ArrayIndexOutOfBoundsException ex) {
					log.debug("charAttributes[" + buffer.charAttributes.length
							+ "," + buffer.charAttributes[0].length + "]="
							+ buffer.windowBase + "," + c);
				}
			...
			if (l >= selectStartLine && l <= selectEndLine) {
				...
				if (selectStartColumn != selectEndColumn) {
					log.debug("select(" + selectStartColumn + "-"
							+ selectEndColumn + ")");
					g.setXORMode(bg);
