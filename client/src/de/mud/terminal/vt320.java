...
public abstract class vt320 extends VDUBuffer implements VDUInput {
	...
	/** the debug level */
	private final static Logger log = LoggerFactory.getLogger(vt320.class);
	...
	public void keyPressed(int keyCode, char keyChar, int modifiers) {
		...
		log.debug("keyPressed(" + keyCode + ", " + (int) keyChar + ", "
				+ modifiers + ")");
	...
	public void keyTyped(int keyCode, char keyChar, int modifiers) {
		...
		log.debug("keyTyped(" + keyCode + ", " + (int) keyChar + ", "
				+ modifiers + ")");
	...
	public void setScreenSize(int c, int r, boolean broadcast) {
		...
		log.debug("setscreensize (" + c + "," + r + "," + broadcast + ")");
	...
	private void putChar(char c, boolean doshowcursor) {
		...
		log.debug("putChar(" + c + " [" + ((int) c) + "]) at R=" + R + " , C="
				+ C + ", columns=" + columns + ", rows=" + rows);
		markLine(R, 1);
		if (c > 255) {
			log.debug("char > 255:" + (int) c);
			// return;
		}
		...
		case TSTATE_DATA:
			...
			if (!useibmcharset) {
				...
				case RI:
					...
					break;
				case IND:
					log.debug("IND at " + R + ", tm is " + tm + ", bm is " + bm);
					if (R == bm || R == rows - 1)
			...
			case '\r':
				...
				break;
			case '\n':
				log.debug("R= " + R + ", bm " + bm + ", tm=" + tm + ", rows="
						+ rows);
				if (!vms) {
				...
				if (usedcharsets) {
					if (c >= '\u0020' && c <= '\u007f') {
						...
						case 'B': // Latin-1 , ASCII -> fall through
							...
							break;
						default:
							log.error("Unsupported GL mapping: " + gx[thisgl]);
							break;
						}
					}
					if (!mapped && (c >= '\u0080' && c <= '\u00ff')) {
						...
						case 'B':
							...
							break;
						default:
							log.error("Unsupported GR mapping: " + gx[gr]);
							break;
						}
					}
				}
		...
		case TSTATE_ESC:
			...
			case 'Y': /* vt52 cursor address mode , next chars are x,y */
				...
				break;
			default:
				log.error("ESC unknown letter: " + c + " (" + ((int) c) + ")");
				break;
			}
			break;
		case TSTATE_VT52X:
		...
		case TSTATE_SETG0:
			if (c != '0' && c != 'A' && c != 'B' && c != '<')
				...
				log.debug("ESC ( : G0 char set  (" + c + " " + ((int) c) + ")");
				gx[0] = c;
			}
			term_state = TSTATE_DATA;
			break;
		case TSTATE_SETG1:
			if (c != '0' && c != 'A' && c != 'B' && c != '<') {
				log.error("ESC ) " + c + " (" + ((int) c) + ") :G1 char set?");
			} else {
				log.debug("ESC ) :G1 char set  (" + c + " " + ((int) c) + ")");
				gx[1] = c;
			}
			term_state = TSTATE_DATA;
			break;
		case TSTATE_SETG2:
			if (c != '0' && c != 'A' && c != 'B' && c != '<')
				log.error("ESC*:G2 char set?  (" + ((int) c) + ")");
			else {
				log.debug("ESC*:G2 char set  (" + c + " " + ((int) c) + ")");
				gx[2] = c;
			}
			term_state = TSTATE_DATA;
			break;
		case TSTATE_SETG3:
			if (c != '0' && c != 'A' && c != 'B' && c != '<')
				log.error("ESC+:G3 char set?  (" + ((int) c) + ")");
			else {
				log.debug("ESC+:G3 char set  (" + c + " " + ((int) c) + ")");
				gx[3] = c;
			}
			term_state = TSTATE_DATA;
			break;
		...
		case TSTATE_DCEQ:
			...
			case 's': // XTERM_SAVE missing!
				log.error("ESC [ ? " + DCEvars[0] + " s unimplemented!");
				break;
			case 'r': // XTERM_RESTORE
				log.debug("ESC [ ? " + DCEvars[0] + " r");
				/* DEC Mode reset */
				for (int i = 0; i <= DCEvar; i++) {
					...
					case 1003:
						...
						break;
					default:
						log.error("ESC [ ? " + DCEvars[0]
								+ " r, unimplemented!");
					}
				}
				break;
			case 'h': // DECSET
				log.debug("ESC [ ? " + DCEvars[0] + " h");
				/* DEC Mode set */
				for (int i = 0; i <= DCEvar; i++) {
					switch (DCEvars[i]) {
					...
					default:
						log.error("ESC [ ? " + DCEvars[0] + " h, unsupported.");
						break;
					}
				}
				break;
			...
			case 'n':
				...
				case 15:
					...
					break;
				default:
					log.error("ESC [ ? " + DCEvars[0] + " n, unsupported.");
					break;
				}
				break;
			default:
				log.error("ESC [ ? " + DCEvars[0] + " " + c + ", unsupported.");
				break;
			}
			break;
		case TSTATE_CSI_EX:
		...
		case TSTATE_CSI:
			...
			case 'i': // Printer Controller mode.
				...
				switch (DCEvars[0]) {
				case 0:
					log.debug("CSI 0 i:  Print Screen, not implemented.");
					break;
				case 4:
					log.debug("CSI 4 i:  Enable Transparent Printing, not implemented.");
					break;
				case 5:
					log.debug("CSI 4/5 i:  Disable Transparent Printing, not implemented.");
					break;
				default:
					log.error("ESC [ " + DCEvars[0] + " i, unimplemented!");
				}
				break;
			case 'l':
				switch (DCEvars[0]) {
				case 4:
					...
					break;
				case 20:
					log.debug("Setting CRLF to FALSE");
					sendcrlf = false;
					break;
				default:
					log.error("ESC [ " + DCEvars[0] + " l, unimplemented!");
					break;
				}
				break;
			case 'A': // CUU
				...
				/* cursor down n (1) times */
			{
				...
				log.debug("to: " + R);
				log.debug("ESC [ " + DCEvars[0] + " B (at C=" + C + ")");
				break;
			}
			case 'C':
				if (DCEvars[0] == 0)
			...
			case 'r': // DECSTBM
				...
				setTopMargin(R);
				_SetCursor(0, 0);
				log.debug("ESC [" + DCEvars[0] + " ; " + DCEvars[1] + " r");
				break;
			...
			case 'H': /* CUP / cursor position */
				...
				log.debug("ESC [ " + DCEvars[0] + ";" + DCEvars[1]
						+ " H, moveoutsidemargins " + moveoutsidemargins);
				log.debug("	-> R now " + R + ", C now " + C);
			...
			case 'f': /* move cursor 2 */
				...
				if (R < 0)
					R = 0;
				log.debug("ESC [ " + DCEvars[0] + ";" + DCEvars[1] + " f");
				break;
			...
			case 'L':
				...
				log.debug("ESC [ " + DCEvars[0] + "" + (c) + " (at R " + R
						+ ")");
				break;
			...
			case 'J':
				...
				log.debug("ESC [ " + DCEvars[0] + " J");
				break;
			case '@':
				log.debug("ESC [ " + DCEvars[0] + " @");
				for (int i = 0; i < DCEvars[0]; i++)
					insertChar(C, R, ' ', attributes);
				break;
			case 'X': {
				int toerase = DCEvars[0];
				log.debug("ESC [ " + DCEvars[0] + " X, C=" + C + ",R=" + R);
				if (toerase == 0)
					toerase = 1;
			...
			case 'P':
				log.debug("ESC [ " + DCEvars[0] + " P, C=" + C + ",R=" + R);
				if (DCEvars[0] == 0)
					DCEvars[0] = 1;
				for (int i = 0; i < DCEvars[0]; i++)
					deleteChar(C, R);
			...
			case 'm': /* attributes as color, bold , blink, */
				...
				log.debug(" (attributes = " + attributes + ")m \n");
				break;
