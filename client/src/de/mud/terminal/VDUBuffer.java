...
public class VDUBuffer {
	...
	public final static Logger log = LoggerFactory.getLogger(VDUBuffer.class);
	...
	public synchronized void insertLine(int l, int n, boolean scrollDown) {
		...
		if (scrollDown) {
			...
			try {
				...
				log.error("*** Error while scrolling up:");
				log.error("--- BEGIN STACK TRACE ---");
				e.printStackTrace();
				log.error("--- END STACK TRACE ---");
				log.error("bufSize=" + bufSize + ", maxBufSize=" + maxBufSize);
				log.error("top=" + top + ", bottom=" + bottom);
				log.error("n=" + n + ", l=" + l);
				log.error("screenBase=" + screenBase + ", windowBase="
						+ windowBase);
				log.error("oldBase=" + oldBase);
				log.error("size.width=" + width + ", size.height=" + height);
				log.error("abuf.length=" + abuf.length + ", cbuf.length="
						+ cbuf.length);
				log.error("*** done dumping debug information");
			}
		}
